  ...
  char txData[] = "Hello from STM32 A\r\n";
   ...
   while (1)
   {
     ...
     if (HAL_UART_Receive(&huart6, (uint8_t*)rxData, sizeof(rxData)-1, 5000) == HAL_OK)
     {
       ...
       int lat = atoi(strip_decimal_new_string(tokens[0]));
       int lon = atoi(strip_decimal_new_string(tokens[1]));
       ...
       int lat2 = 432814000;
                ...
                if (abs(lat - lat2) < 18000 && abs(lon - lon2) < 18000){
             	   printf("%s\r\n", "b");
                ...
                if (abs(lat - lat2) < 12000 && abs(lon - lon2) < 12000){
                             	   printf("%s\r\n", "b");
