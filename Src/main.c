...
int main(void)
{
  ...
  char txData[] = "Hello from STM32 A\r\n";
   char rxData[100];
   uint8_t rxGPS = 0;
   uint8_t idx = 0;
   char rxBuffer[100];
   ...
   while (1)
   {
	 ...
	 if (HAL_UART_Receive(&huart1, &rxGPS, 1, HAL_MAX_DELAY)){
		 if (rxGPS != '\n'){
			 rxBuffer[idx++] = rxGPS;
		 }
		 else{
 			...
 			if (strstr(rxBuffer, "$GPRMC") != NULL && strstr(rxBuffer, "N") != NULL  && strstr(rxBuffer, "W") != NULL){
 				...
 				int lat2 = atoi(strip_decimal_new_string(tokens[lat_idx]));
 				int lon2 = atoi(strip_decimal_new_string(tokens[lat_idx]));
     ...
     if (HAL_UART_Receive(&huart6, (uint8_t*)rxData, sizeof(rxData)-1, 5000) == HAL_OK)
     {
       ...
       lat = atoi(strip_decimal_new_string(tokens[0]));
       lon = atoi(strip_decimal_new_string(tokens[1]));
